...
func Start(channel ssh.Channel, args []string) {
	...
	// validate arguments
	if cmdIps == "" {
		lg.Error("[scan] Unspecified or blank --ips flag")
		channel.Write([]byte("[scan] Unspecified or blank --ips flag\n"))
		return
	}
	if cmdThreads <= 0 {
		lg.Error("[scan] Invalid value for flag --threads")
		channel.Write([]byte("[scan] Invalid value for flag --threads\n"))
		return
	}
	if cmdTimeout <= 0 {
		lg.Error("[scan] Invalid value for flag --timeout")
	...
	if len(ports) == 0 {
		lg.Error("[scan] No ports to scan")
...
func (s *subsystemPscanConfig) Scan(ctx context.Context) {
	...
	lg := logger.GetLogger()
	lg.Info("[scan] Start new scan on ips %v with ports %v on %d threads", s.ips, s.ports, s.threads)
